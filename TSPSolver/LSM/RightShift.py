...
class right_shift_move(LocalSearchMove):
    ...
    def set_gain(self, data: TSPInstance) -> None:
        ...
        self._gain += data.get_cost(seq[self.j - 1], seq[y])

    def perform(self) -> None:
        if self._gain < 0:
            for k in range(self._degree + 1):
                move(self.i if self._withTwoOpt else self.i + k, self.j + k).right_shift(self._sequence)
