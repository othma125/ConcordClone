...
class Location:
    ...
    def to_geo(self):
        x = Location.to_geo_radians(self.latitude)
        y = Location.to_geo_radians(self.longitude)
        return Location(x, y)
    ...
    def get_geo_great_circle_distance(self, location: 'Location') -> int:
        ...
        q1 = math.cos(self.longitude - location.longitude)
        q2 = math.cos(self.latitude - location.latitude)
        q3 = math.cos(self.latitude + location.latitude)
    ...
    def get_euclidean(self, location: "Location") -> float:
        dx = self.latitude - location.latitude
        dy = self.longitude - location.longitude
