...
def calculate_gap(file_name: str, tour: TSPTour, data_frame: pd.DataFrame) -> pd.DataFrame:
    """Calculate the gap between the best known solution and the current solution"""

    instance_name = file_name.split('.')[0]
    best_known_row = data_frame[data_frame['Instance'] == instance_name]
    ...
    gap = (float(tour.cost) - best_known_value) / best_known_value if best_known_value != 0 else 0
    # Create a dictionary as result
    rslt = {'file_name': file_name,
            'best_known_value': float(best_known_value),
            'best_known_time(s)': best_known_time,
            'method_value': float(tour.cost),
            'method_time(s)': round(tour.reach_time, 2),
            'method_name': tour.method,
            'stops_count': len(tour.sequence),
            'gap': f"{gap:.2%}"}
    return rslt
